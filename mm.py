...
GROUP_ID = "-1002447886308"
CHANNEL_USERNAME = "@RitikXyz099"
...
def handle_attack(message):
    ...
    if not is_user_in_channel(user_id):
        bot.reply_to(message, f"𝐉𝐎𝐈𝐍 𝐊𝐑𝐎 𝐏𝐀𝐇𝐋𝐄 {https://t.me/hackssll} ░░░░✦╬═••")
        return

    # ✅ Check if the user has pending feedback
    if pending_feedback.get(user_id, False):
        bot.reply_to(message, "☬𝐀𝐓𝐓𝐀𝐂𝐊 𝐋𝐀𝐆𝐀𝐘𝐀 𝐍𝐀 ☫𝐁𝐀𝐇𝐄𝐍𝐂𝐇𝐎𝐃 ❦𝐀𝐁 𝐒𝐂𝐑𝐄𝐄𝐍 𝐒𝐇𝐎𝐓 ☬𝐃𝐄 𝐓𝐀𝐁 𝐀𝐆𝐋𝐀 𝐀𝐓𝐓𝐀𝐂𝐊 𝐋𝐆𝐄𝐆𝐀 ☬")
        return
    ...
    if time_duration > 120:
        bot.reply_to(message, "Error: Attack duration cannot exceed 120 seconds.")
        return
    ...
    bot.send_photo(message.chat.id, random_image, 
                   caption=f"🚀  {user_name}, 𝘼𝙩𝙩𝙖𝙘𝙠 𝙨𝙩𝙖𝙧𝙩𝙚𝙙 𝙤𝙣 `{target} : {port}`\n"
                           f"⏳ **Remaining: {time_duration} seconds**\n"
...
def send_attack_finished(message, user_name, target, port, time_duration, remaining_attacks):
    ...
    bot.send_message(message.chat.id, 
                     ...
                     f"⏳ **Attack Duration: {time_duration} seconds**\n"
                     f"⚡ 𝙍𝙀𝙈𝘼𝙄𝙉𝙄𝙉𝙂 𝘼𝙏𝙏𝘼𝘾𝙆𝙎 = {remaining_attacks}")
